...
@interface SigninVC ()<UITextFieldDelegate, GIDSignInDelegate>
...
- (void)viewDidLoad
{
    ...
    if([Util getStringData:kAppNameWhiteImage] !=nil){
        ...
        [self.imgHeaderImage sd_setImageWithURL:[Util EncodedURL:[Util getStringData:kAppNameImage]] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (image == nil)
            {
                self.imgHeaderImage.image = [UIImage imageNamed:@"imgLogoCiyaShop"];
            }
            else
            {
...
didSignInForUser:(GIDGoogleUser *)user
  ...
  if (error == nil) {
    ...
    [FIRGoogleAuthProvider credentialWithIDToken:authentication.idToken
      ...
      NSURL *imgURL = [user.profile imageURLWithDimension:200];
      [NSURLConnection sendAsynchronousRequest:[NSURLRequest requestWithURL:imgURL] queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError)
      {
          ...
          [dict setValue:@"-" forKey:@"birthday"];
          [dict setValue:user.profile.givenName forKey:@"first_name"];
          [dict setValue:user.profile.familyName forKey:@"last_name"];
          [self socialLogin:dict imgData:data];
      }];
